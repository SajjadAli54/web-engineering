...
function printHobbies(){
    users.forEach(obj => console.log(obj.hobbies));
}

function findHometownByState(state){
    return users.find(obj => obj.hometown.state == state).hometown;
}
...
function allLanguages(){
    ...
    users.forEach(obj => {
        for(let x of obj.favoriteLanguages)
            if(!lang.includes(x))
                lang.push(x);
    })
    return lang;
...
console.log(findByUsername('anne'));
